...
impl Plugin for EndPlugin {
    fn build(&self, app: &mut AppBuilder) {
        app.add_system_set(SystemSet::on_enter(Screen::End).with_system(setup.system()))
            .add_system_set(SystemSet::on_update(Screen::End).with_system(input.system()))
            .add_system_set(SystemSet::on_exit(Screen::End).with_system(despawn.system()));
    }
}
...
fn input(mut input_state: ResMut<Input<KeyCode>>, mut game_state: ResMut<State<Screen>>) {
    if input_state.just_pressed(KeyCode::Space) {
        game_state.set(Screen::Title).unwrap();
